(ns improbable.main
  "Kotodama WASM entrypoint for the Improbable clean-room actor (L4) — Clojure port.
  ...
  facts (`improbable.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "improbable")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"improbable.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "improbable-compat" :tier tier :entities entities} 200])
...
(defn start! [] :improbable-compat/ready)
